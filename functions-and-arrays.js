...
function averageWordLength (array) {
  var result = 0;
  var promedio = words.length;

  for (var i = 0 ; i < words.length;i++){
    var contador = words[i].length;
    result += contador;
...
for (var i = 0; i < words.length; i++){
  element = words[i];
  var index = words.indexOf(element, i + 1);
  if (index > 0) {
...
//   for (var i = 0; i < words.length; i++){
//     element = words[i];
//     var preResult = words.indexOf(element);
//     while (preResult != -1){
//       result[i] = words[i];
...
var howManyMatter = howManyTimes(palabras, "matter");
console.log(howManyMatter);

var howManyDog = howManyTimes(palabras, "dog");
